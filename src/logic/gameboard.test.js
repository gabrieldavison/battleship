...
describe('tests recieveAttack', () => {
  ...
  test('revieveAttack records missed attackonto board array', () => {
    newBoard.recieveAttack(4,4)
    expect(newBoard.board[44].hits).toBe('o')
  })

  test('revieveAttack records successful attack onto board array', () => {
    newBoard.placeShip(3, 4, 5 , 'h');
    newBoard.recieveAttack(5,5)
    expect(newBoard.board[55].hits).toBe('x')
  })
