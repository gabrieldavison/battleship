...
test('player.taketurn attacks gameboard', () => {
  testPlayer1.takeTurn(5,5, testPlayer2.gameboard)
  expect(testPlayer2.gameboard.board[55].hits).toBe('o');
})
...
describe('tests using randomTurn', () => {
  ...
  test('randomTurn logs a random attack to board', () => {
    mockRnd.mockReturnValueOnce(3).mockReturnValueOnce(4)
    testPlayer2.randomTurn(testPlayer1.gameboard, mockRnd)
    expect(testPlayer1.gameboard.board[43].hits).toBe('o');
  })
  
  test('randomTurn logs its coordinates in turns array', () => {
    expect(testPlayer2.turns[0]).toEqual([3,4])
  })

  test('isLegal returns false if space has already been attacked', () => {
    expect(isLegal(testPlayer1.gameboard, [3,4])).toBe(false)
  })

  test('randomTurn wont play the same coordinates twice', () => {
    mockRnd.mockReturnValueOnce(3).mockReturnValueOnce(4)
    .mockReturnValueOnce(3).mockReturnValueOnce(4)
    .mockReturnValueOnce(3).mockReturnValueOnce(4)
  .mockReturnValueOnce(4).mockReturnValueOnce(5)
    testPlayer2.randomTurn(testPlayer1.gameboard, mockRnd)
    expect(testPlayer2.turns[1]).toEqual([4,5])
  })
})
